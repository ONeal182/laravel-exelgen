...
class ExelGen extends Controller
{
    ...
    public function formRequest($bdData, Request $request, $dataBD = false)
    {
        if (!$dataBD) {
            //Данные из формы
    ...
    {
        $date = $this->formRequest(null, $request);
    ...
    public function generateExcel($bdData, Request $request, $dataBD = false)
    {
        ...
        foreach ($this->wordBreak($contractorName . ' ОРГН ' . $contractorORGN . ' ИНН ' . $contractorINN . ' ' . $contractorAddres . ' тел. ' . $contractorPhone . ' факс. ' . $contractorFax, 155) as $text) {
            $this->i++;
            $this->creatRow(
                [
                    ['row' => 'A' . $this->i . ':I' . $this->i . '', 'text' => $text, 'style' => $styleText]
        ...
        foreach ($date['workDo'] as $key => $value) {
            foreach ($this->wordBreak($value, 145) as $text) {
                $this->i++;
                $this->creatRow(
                    [
                        ['row' => 'A' . $this->i . ':I' . $this->i . '', 'text' => $text, 'style' => $styleText]
        ...
        foreach ($this->wordBreak($razdeDoc . ' ' . $RegName, 145) as $text) {
            $this->i++;
            $this->creatRow(
                [
                    ['row' => 'A' . $this->i  . ':I' . $this->i  . '', 'text' => $text, 'style' => $styleText]
